...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        File file = new File(root, URLDecoder.decode(request.getUri(), StandardCharsets.UTF_8.toString()));
        if (!file.isFile()) {
            if (file.isDirectory())
        ...
        if (file.isFile()) {
            ...
            if (request.headers.containsKey("If-None-Match") && etag.equals(request.getHeader("If-None-Match"))) {
